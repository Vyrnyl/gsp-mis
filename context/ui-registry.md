...
| Activity Reports screen | **built** | `src/features/activity-reports/components/activity-reports-view.tsx` | Feature 2.3. `/activity-reports` — one screen, two role-driven variants: Troop Leader's own-scoped "My Activity Reports" + Submit action, Admin/Executive Council's all-troops table (Submitted By/Troop columns), read-only for Executive Council. Submit form modal (`activity-report-form-modal.tsx`) and read-only detail modal (`activity-report-detail-modal.tsx`) reuse existing kit primitives only |
| Badges screen | **built** | `src/features/badges/components/badges-view.tsx` | Feature 2.4. `/badges` — stat cards + `Tabs`-based Badge Catalog / Member Progress / Achievement History, plus a page-level Record Badge action. Catalog CRUD (`badge-catalog-panel.tsx`, `badge-form-modal.tsx`) gated `canManage` (Admin only); recording badges/achievements (`record-badge-modal.tsx`, `achievement-form-modal.tsx`) gated `canRecord` (Admin + Troop Leader); verifying an earned badge (`member-progress-detail-modal.tsx`) gated `canVerify` (Admin only). Executive Council reads only, all three tabs |
| Announcements screen | **built** | `src/features/announcements/components/announcements-view.tsx` | Feature 2.5. `/announcements` — paginated feed of `AnnouncementCard`s, `New Announcement` action (`announcement-form-modal.tsx`) gated `canPost` (Admin + Executive Council, matching `announcements:write`); Troop Leader reads only |
| Notifications screen | **built** | `src/features/notifications/components/notifications-view.tsx` | Feature 2.5. `/notifications` — full paginated personal history, reached only via the topbar bell's "View All Notifications" link (no sidebar nav entry) |
| Finance screen | **built** | `src/features/finance/components/finance-view.tsx` | Feature 3.1. `/finance` — 4-tab screen (Overview / Payments / Expenses / Fee Types) on `Tabs`. Overview: stat cards + `FinanceTrendChart` (income vs. expense bar) + `ExpenseCategoryDonut` + Budget & Collection Summary card. Payments/Expenses are paginated, append-only (Record actions only, no edit/delete — matches the granted verbs). Fee Types is Admin CRUD with a usage-guarded delete (`fee-types-panel.tsx`, same pattern as 1.6's `CategoryPanel`). All write actions gated `canManage` (Admin only, per the pre-build fix removing `finance:write` from Executive Council); Executive Council reads all four tabs |
| Reports screen | **built** | `src/features/reports/components/reports-view.tsx` | Feature 3.2. `/reports` — one page covering all 6 types (Membership/Attendance/Badge/Financial/Activity/Executive): `ReportTypeSelector` (radio-card grid, filtered per role/permission by `getAvailableReportTypes`), `ReportFiltersBar` (date range + optional Troop `Select`, Admin/EC only), `ReportPreviewPanel` (generic stat-cards + table renderer shared by every type since `columns`/`rows` are already shaped server-side), `ReportHistoryPanel` (paginated table of past exports, reads the `Report` model). Export (`reports:export`) is Admin + Executive Council only — Troop Leader sees a "Ask an administrator or council member to export this report" note instead of the Export buttons. Financial/Executive types are hidden from Troop Leader's type selector (no `finance:read`/role-gated) and blocked server-side too, including on History and direct-by-id download |
...
| Component                | Status        | File                                 | Exact classes                                                                                                                                                                                                                                                    |
| ------------------------ | ------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stat card                | **built**     | `src/shared/components/ui/stat-card.tsx` (+ `StatCardSkeleton` in `skeleton.tsx`) | Feature 1.5. `<StatCard icon value label tone>`. Replaces `.stats-grid`/`.stat-card`/`.stat-icon`/`.stat-info` — tone (`green\|gold\|blue\|red`) sets the left border + icon tint via the same `status-*-bg` tokens `Badge` uses. **Fixed in 3.1**: added `min-w-0`/`break-words` on the value — long currency strings (e.g. "₱20,500") were overflowing into the next card in a 4-up grid at exactly 900px, a case member-count/percentage values never hit |
...
| Badge / achievement card | **built**     | `src/features/badges/components/badge-catalog-panel.tsx` | Feature 2.4. Not a per-viewer `.earned` toggle like the prototype — catalog cards are neutral catalog info (name/category/points/earned-count); "earned" is a per-member fact shown on the Member Progress tab (`member-progress-panel.tsx`) via `MemberBadgeStatusBadge` (`member-badge-status-badge.tsx`) instead |
...
| Announcement card        | **built**     | `src/features/announcements/components/announcement-card.tsx` | Feature 2.5. Built on `Card` — title/content, "Posted by … · <relative time>", an "Expires <date>"/"Expired" `Badge` when `expiresAt` is set |
...
| Bar chart (Income vs. Expense) | **built** | `src/features/finance/components/finance-trend-chart.tsx` | Feature 3.1. Two-dataset `<Bar>` (Income/Expense), `Legend` registered bottom-position |
| Donut chart (Expenses by Category) | **built** | `src/features/finance/components/expense-category-donut.tsx` | Feature 3.1. `<Doughnut>` + hand-built legend list, `legend:{display:false}` explicitly set since `Legend` is a shared ChartJS registry |
| Bar chart (Analytics — 6 variants) | **built** | `src/features/analytics/components/{membership-trend-chart,attendance-trend-chart,participation-chart,badge-completion-chart,financial-trend-chart,organization-performance-chart}.tsx` | Feature 3.3. Vertical `<Bar>` for Membership/Attendance/Organization/Financial trends; **first horizontal-bar use** (`indexAxis:'y'`) for Participation and Badge Completion, since event titles/badge names read better as row labels than crowded x-axis ticks. All six explicitly set `legend:{display:false}` except `FinancialTrendChart` (the only one with two datasets) — required because all six are statically imported on one page, so `Legend`'s registration is a shared ChartJS registry, not per-chart (same bug class as the finance donut) |
...
| Report type selector    | **built** | `src/features/reports/components/report-type-selector.tsx` | Feature 3.2. Not the prototype's `onclick`-`<div>` cards — real radio group (`sr-only` inputs + styled card labels), same technique as `RoleSelector`, so it gets keyboard/screen-reader semantics for free |
...
| Notification panel | **built** | `src/features/notifications/components/notification-bell.tsx` | Feature 2.5. Replaces `.notif-panel`/`.notif-item`/`.notif-icon`/`.notif-footer`. Topbar bell + dropdown, real fetch on mount, mark-one/mark-all-read, a real "View All Notifications" link to `/notifications` (the prototype's was dead text). Fixed 320px, right-anchored, capped `calc(100vw-2rem)` — verified not to overflow at 360px |
| Notification row | **built** | `src/features/notifications/components/notification-row.tsx` | Feature 2.5. Not `ActivityItem` — that component's `meta` slot is a one-line caption; a notification needs a full wrapping title *and* message body. Built from `ActivityDot` for the unread indicator |
...
| Announcement form modal | **built** | `src/features/announcements/components/announcement-form-modal.tsx` | Feature 2.5. Title/Content/optional Expires-On (native `<input type="date">`, matching 1.3/2.1's precedent). Create-only for v1 — no edit/delete (build-plan.md §2.5 scope) |
...
| ---------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-24 | Finance (Feature 3.1, full loop) | Built `finance-view.tsx` (`/finance`, 4 tabs: Overview/Payments/Expenses/Fee Types), `finance-overview-panel.tsx`, `payments-panel.tsx` + `payment-form-modal.tsx` + `payment-status-badge.tsx`, `expenses-panel.tsx` + `expense-form-modal.tsx`, `fee-types-panel.tsx` + `fee-type-form-modal.tsx`, and two new charts (`finance-trend-chart.tsx` — income vs. expense bar, `expense-category-donut.tsx`) in `src/features/finance/`. No new base primitives — composes `Card`, `Table`, `Modal`, `ConfirmDialog`, `FormField`, `Select`, `Pagination`, `Tabs`, `StatCard`, `EmptyState`/`ErrorState`/`TableSkeleton`/`ChartSkeleton`/`StatCardSkeleton`. Added `IncomeIcon`/`ExpenseIcon`/`BalanceIcon` to the shared icon barrel. Pre-build RBAC fix: removed `finance:write` from Executive Council in both `roles.ts` files (project-overview.md scopes Executive Council to read-only "Financial Monitoring"; only Administrator "Records payments/expenses"). **3 real bugs caught during self-review**: (1) `FinanceTrendChart` registering ChartJS's `Legend` plugin is a global registry, not per-chart — it silently turned on `ExpenseCategoryDonut`'s own built-in legend too, colliding with its custom HTML one, fixed by explicitly setting `legend: {display:false}` on the donut; (2) the step-1 "Empty" preview passed `overview: null`, which `FinanceOverviewPanel` treats as "still loading" — showed a permanent skeleton instead of a real empty state, fixed with a genuinely-zeroed `EMPTY_FINANCE_OVERVIEW` fixture; (3) recording a payment didn't refetch Fee Types, so the payment-count usage-guard (which gates fee-type deletion) went stale until a full reload. **Also fixed a pre-existing bug inherited from the `event-form-modal.tsx`/`category-form-modal.tsx` pattern this feature's 3 modals were built from**: `isSubmitting` was never reset on a successful submit, so Cancel/Save became permanently disabled the next time the same modal instance reopened — fixed in finance's 3 modals only (the shared pattern in 1.6/2.1 is a known follow-up, not fixed here). Deviations: Payments/Expenses are append-only (no edit/delete — matches the granted "Record payments/expenses" verbs, same precedent as 2.3's activity reports); Fee Types uses a bespoke panel rather than 1.6's generic `CategoryPanel` (fee types carry an `amount` field categories don't). |
| 2026-07-23 | Activity Reports (Feature 2.3, full loop) | Built `activity-reports-view.tsx` (`/activity-reports`), `activity-report-form-modal.tsx`, `activity-report-detail-modal.tsx`, `activity-report-status-badge.tsx` in `src/features/activity-reports/`. No new base primitives — composes `Card`, `Table`, `Modal`, `FormField`, `Select`, `Textarea`, `Pagination`, `EmptyState`/`ErrorState`/`TableSkeleton`. Added `DocumentIcon` (`FiFileText`) to the nav for this feature's entry. New route not among Phase 0's original placeholders — added directly. Reuses 2.1's `listAllEvents({status:'completed'})` for the report's event picker rather than a new lookup. Deviation: no "mark reviewed" action — build-plan.md scopes exactly 3 endpoints (no PATCH) and Council/read-only, so `reviewed` stays a reachable-in-schema-only status. |
