...
**Last updated:** 2026-07-23 · **Current phase:** Phase 1 — Core Operations · **Current feature:** 1.3 Membership Management is done (●) — next up is **1.4 Membership Approval (Executive Council)**
...
| Phase 1 — Core Operations | 6        | 3      | 0     | 0     | 0     | 3     | 0     |
| Phase 2 — Activities      | 5        | 5      | 0     | 0     | 0     | 0     | 0     |
| Phase 3 — Finance & Admin | 5        | 5      | 0     | 0     | 0     | 0     | 0     |
| **Total**                 | **21**   | **13** | **0** | **0** | **0** | **8** | **0** |

**Overall completion: 8 / 21 features done (38%) — Phase 0 complete, 1.1 Authentication, 1.2 App Shell + Role-Based Navigation, and 1.3 Membership Management all done end-to-end. Next: 1.4 Membership Approval (Executive Council).**
...
| --------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------- |
| RBAC enforced per route                                   | ◑      | `requireAuth`/`requireRole` live on `GET /auth/me` (1.2) and all 7 members + 2 organizations routes (1.3, `members:archive` correctly restricts archive/restore to admin + executive council — curl-verified 403 for troop leader). Next.js middleware still UX-layer only (unverified token decode) |
| Validation (frontend + backend)                           | ●      | Backend: auth (5 schemas) + members (`memberType`-discriminated create/update, query, renew) all Zod, live on real routes; `ZodError` → 422 with field details. Frontend: `FormField` renders field errors on login/signup/member-form, incl. every role/type-specific field |
...
| Responsive at 900px / 768px / 480px                       | ◑      | Mobile-first with a custom `lg2: 900px` stop. **Human-verified 2026-07-22** on the gallery + shell; re-checked per feature screen (1.3 directory + profile re-confirmed with real data 2026-07-23) |
...
| Empty / loading / error states on every list              | ●      | 1.3's Membership Registry is the first real list — empty (no data + no filter matches), loading, and error (retry) all verified live |
| Pagination on tables that can grow                        | ●      | `Pagination` built, unit-tested, and now wired to a real server-paginated list (1.3, page/pageSize query params)                  |
| Unit tests on service layer                               | ●      | 36 tests passing (31 API, 5 web): health service, error handler, `asyncHandler`, `buildPageRange`, `authService` (14 tests), and now `membersService` (9 tests — list/getById/create/update/archive/restore/renew) |
...
**One open decision (#7, email-sending service) — non-blocking. 1.1 Authentication, 1.2 App Shell + Role-Based Navigation, and 1.3 Membership Management are all done (●); 1.4 Membership Approval is next and does not depend on it.**
