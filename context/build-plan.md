...
4. ● **DONE (uncommitted) — Participation** — by activity type and by school with the active-vs-inactive split, plus the **Community Engagement** card that R3 missed. Community is matched by category *name* (the schema has no flag), so the card states which categories it counted and its stated definition is read from the council's full vocabulary rather than from the filtered rows — a filter must change the numbers, never what they claim to be.
